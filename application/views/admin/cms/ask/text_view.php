...
<main class="main-content p-5" role="main">
    ...
    <div class="row mb-5">
        <div class="col-md-12">
          ...
          <a class="btn btn-info" href="<?=site_url('admin/Cms/EditSub/rqwerewewdfstefedfsaasfdfd8s9f7ds8f74d56s4fsd564f5d6sf')?>" ><?=lang('am_Edit_data').' '.$Name ?><span class="fa fa-edit"></span></a>
                                            <a class="btn btn-info" href="<?= site_url('admin/Cms/add_ask/'.$sub_Token)?>" ><?=lang('am_add_item');?></a>
                                        ...
                                        <div class="table-responsive"> 
                                          <table id="example" class="table table-bordered table-striped table-hover " cellspacing="0" width="100%">
                                                   <thead>
                                                            <tr>
                                                                <th>  </th>
                                                                <th><?= lang('am_question')?> </th>
                                                                <th style="width:45%"><?= lang('am_answer')?></th> 
                                                                <th><?= lang('date') ?></th>
                                                                <th></th>
                                                            </tr>
                                                    </thead>
                                                    <tbody>
                                                        <?php foreach($items as $item): ?>
                                                            <tr>    
                                                                ...
                                                                <td><?=$item->Date;?></td>
                                                                <td>
                                                                    <a class="btn btn-sm btn-info" href="<?=site_url();?>admin/Cms/edit_ask/<?=$item->Token;?>" ><span class="fa fa-edit"></span></a>
                        ...
                        success : function(response){
                            //window.location
                             if(response.status == 1 ){
                                window.location = '<?= site_url();?>admin/Cms/getDep_ask/'+depToken;
                             }else{
